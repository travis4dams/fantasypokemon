...
def is_gen2_dex_num(n):
	return is_int(n) and n > 0 and n < 252

def get_timestamp_str():
	return str(datetime.fromtimestamp(datetime.timestamp(datetime.now()))).split('.')[0].replace(':','.').replace(' ','_')
...
# returns the string name of the parent function of the function that called get_parent_fname
# i.e. f() -> g() -> get_parent_fname() returns "f"
...
def tab_format(inlist):
	...
	# using the actual "\t" character seems more inconsistent than an arbitrary number of spaces
	tab_postfix = " "
	...
	for instr in inlist:
		instr_len = len(instr)
		# get the nearest multiple of 8 based on instr_len
		#adjusted_instr_len = math.ceil( ( instr_len / 8 ) ) * 8
		num_tabs_needed = math.floor( (max_len - instr_len) / 8 )
		# num_tabs_needed = num_tabs_needed + 1 # tab_format() assumes each str should be appended by tab at least once
		printd("num_tabs " + str(num_tabs_needed))
		result.append(instr + "\t"*(num_tabs_needed+1) )
	printd(result)
	return result
	"""
...
def get_all_resets(): 
	return get_color("reset") + get_bg_color("reset") + get_style("reset")
