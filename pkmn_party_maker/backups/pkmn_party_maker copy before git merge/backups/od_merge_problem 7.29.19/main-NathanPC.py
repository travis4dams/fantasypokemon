...
def team_edit(team_num, slot_num, is_legendary = False):
	...
	for i in range(len(team)):
		if i != slot_num:
			new_team_ids.append(team[i]['dexnum'])
...
def team_show(team_num = None):
	...
	for i in range(len(teams)):
		...
		for j in range(len(team)):
			...
			pkmn = team[j]
			pid = pkmn.get_dexnum()
			pkmn_name = pkmn.get_species_name() + " [#" + str(team[j]['dexnum']) + "]"
...
def load_team_file(fname):
	...
	# if user did not supply a fname, attemot to open default file
	else:		
		if i == 0:
...
def init_static_pkmn_vars(pkmn):
	...
	for label in stat_labels:
		pkmn.set_ev(65535, label)
		pkmn.set_iv(15, label)
