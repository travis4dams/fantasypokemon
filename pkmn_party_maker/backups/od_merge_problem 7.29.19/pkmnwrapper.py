...
# maps pkmn dex_nums to pokemon objs that have already been loaded
...
def client_pkmn_request(dex_num):
	# check input
	assertd(is_int(dex_num),"Bad input to client_pkmn_request(): " + str(dex_num) + " is not an int.")
	assertd(is_gen2_dex_num(dex_num),"dex_num to client_pkmn_request() is out of range: " + str(dex_num))

	# client request
	try:
		return client.get_pokemon(dex_num)
	except beckett.exceptions.InvalidStatusCodeError:
		raise ValueError("Bad input to client.get_pokemon() in get_PKMN() wrapper: " + str(dex_num))
	
	# generic catch... but commented out because why not just pass the original error for more info about the error?
	"""
	except:
		raise Exception("Unknown error in get_PKMN() fetching dex_num " + str(dex_num))
		#squit()
	"""

# input: 1 <= dex_num <= 251
# output: Pokepy Pokemon object given an int Pokedex number
# NOTE: only Gen2 dex_num's are allowed (1-251 inclusive)
# TODO: string name input
def get_PKMN(dex_num):
	# check input
	assertd(is_int(dex_num),"Bad input to get_PKMN(): " + str(dex_num) + " is not an int.")

	# check if pkmn obj is cached
	if(dex_num in pkmn_obj_cache):
		return pkmn_obj_cache[dex_num]

	# get output, check it, and return if acceptable
	result = client_pkmn_request(dex_num)	
...
def get_PKMN_name(dex_num):
	# check input
	assertd(is_int(dex_num),"Bad input to get_PKMN_name(): " + str(dex_num) + " is not an int.")
	assertd(is_gen2_dex_num(dex_num),"dex_num to get_PKMN_name() is out of range: " + str(dex_num))

	# attempt to get pokemon object and its name
	result_pkmn = get_PKMN(dex_num)
...
def get_moves(p, g = 2, include_prior_evos = True):
	...
	dex_num = p.id
	#printd(dex_num)
	#printd(pkmn_move_cache)
	#input()
	if(dex_num in pkmn_move_cache):
		printd("move cache hit")
		result = pkmn_move_cache[dex_num][1]
		return result
	...
	for move in moves:
		...
		if move_id in rs.inferior_moves_dict:
			...
			"""
				...
				printd("Detected inferior move for " + get_PKMN_name(dex_num).capitalize() + ": " + inf_mname + " to " + sup_mname)
			"""
		else:
			moves_result.append(move)
...
def get_evolution_chain(dex_num):
	assertd(is_int(dex_num),
		"Bad input to get_evolution_chain(); dex_num is not a list. Type: " + get_class_name(dex_num) + " str: " + str(dex_num))
...
def get_type_color(type_name):
	#printd("get_type_color: " + str(type_name))
